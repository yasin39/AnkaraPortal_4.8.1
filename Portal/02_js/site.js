...
// ==> TC Kimlik sorgulama fonksiyonu - Server-side entegrasyonlu
tcKimlikSorgula() {
    const tcKimlik = document.getElementById('tcKimlik').value;
    ...
    // ==> Server-side'a veri gönder
    document.getElementById('<%= hdnPersonelData.ClientID %>').value = JSON.stringify({
        tcKimlik: tcKimlik,
        islemTipi: 'TC_SORGULA'
    });

    document.getElementById('<%= hdnIslemTipi.ClientID %>').value = 'TC_SORGULA';

    // Server-side butonu tetikle
    document.getElementById('<%= btnHiddenTCSorgula.ClientID %>').click();
}